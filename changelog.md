...
- Fixed an issue where song names in the Eclipse Menu integration would not update when randomizing songs on level exit.
## v1.13.0
*<c-aaaaaa>Friendly reminder that</c> <cl>some</c> <cj>features</c> <c-aaaaaa>of this mod (including features from this update and features related to bugfixes in this update) require [Better Touch Prio](mod:alk.better-touch-prio) to be installed. These decisions are still here to stay.</c>*
...
- <cg>Added</c> numerous keybinds inspired by <cr>YouTube</c>, <c-FFA544>VLC</c>, and <c-06CE91>Spotify</c> into both the Control Panel and the Song List menus.
  ...
  - <c-06CE91>Cmd + H</c> (macOS)/<c-06CE91>Ctrl + H</c> (Windows): Go from Song List menu to Control Panel
  - <c-06CE91>Cmd + K</c> (macOS)/<c-06CE91>Ctrl + K</c> (Windows): Switch between Control Panel and Song List
  ...
  - You can toggle (all of) these keybinds off in the mod settings.
...
- <co>Fixed</c> issues with detecting Newgrounds/Music Library songs stored in non-vanilla folders.
  - Try to name your files using both letters and numbers to reduce false positives.
