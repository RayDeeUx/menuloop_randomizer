...
class $modify(MenuLayerMLHook, MenuLayer) {
	static cocos2d::CCScene* scene(bool p0) {
		...
		if (!shownVibecodedWarning && ((SongManager::get().getVibecodedVentilla() && Loader::get()->isModLoaded("joseii.ventilla")) || (SongManager::get().getVobecidedJota() /* leaving this in is fine as it will eval to false anyway */ /* && Loader::get()->isModLoaded("jotabelike.foobarbaz") */ )) && scene->getChildByType<MenuLayer>(0)) {
